...
#include <iostream>
#include <Examples\AugmentedModel\ecircdsoption.hpp>
#include <Examples\AugmentedModel\ecircdsoptiontests.hpp>
    ...
    */
    ...
    for (Size i = 0; i < 4; i++) {
        instruments.push_back(ext::shared_ptr<DefaultProbabilityHelper>(new SpreadCdsHelper(
            Handle<Quote>(ext::shared_ptr<Quote>(new SimpleQuote(quoted_spreads[i]))), tenors[i], 0,
            calendar, Quarterly, Following, DateGeneration::TwentiethIMM, Actual365Fixed(),
            recovery_rate, tsCurve)));
    }

    // Bootstrap hazard rates
    ext::shared_ptr<PiecewiseDefaultCurve<HazardRate, BackwardFlat> > hazardRateStructure(
        new PiecewiseDefaultCurve<HazardRate, BackwardFlat>(todaysDate, instruments,
                                                            Actual365Fixed()));
    ...
    for (Size i = 0; i < hr_curve_data.size(); i++) {
        cout << "hazard rate on " << hr_curve_data[i].first << " is " << hr_curve_data[i].second
             << endl;
    }
    cout << endl;
    ...
    cout << "1Y survival probability: "
         << io::percent(hazardRateStructure->survivalProbability(todaysDate + 1 * Years)) << endl
         << "               expected: " << io::percent(0.9704) << endl;
    cout << "2Y survival probability: "
         << io::percent(hazardRateStructure->survivalProbability(todaysDate + 2 * Years)) << endl
         << "               expected: " << io::percent(0.9418) << endl;
    ...
    CreditDefaultSwap cds_3m(Protection::Seller, nominal, quoted_spreads[0], cdsSchedule, Following,
                             Actual365Fixed());
    ...
    cdsSchedule = MakeSchedule()
                      ...
                      .withTerminationDateConvention(Unadjusted)
                      .withRule(DateGeneration::TwentiethIMM);
    CreditDefaultSwap cds_6m(Protection::Seller, nominal, quoted_spreads[1], cdsSchedule, Following,
                             Actual365Fixed());
    ...
    cdsSchedule = MakeSchedule()
                      ...
                      .withTerminationDateConvention(Unadjusted)
                      .withRule(DateGeneration::TwentiethIMM);
    CreditDefaultSwap cds_1y(Protection::Seller, nominal, quoted_spreads[2], cdsSchedule, Following,
                             Actual365Fixed());
    ...
    cdsSchedule = MakeSchedule()
                      ...
                      .withTerminationDateConvention(Unadjusted)
                      .withRule(DateGeneration::TwentiethIMM);
    CreditDefaultSwap cds_2y(Protection::Seller, nominal, quoted_spreads[3], cdsSchedule, Following,
                             Actual365Fixed());
...
void example02() {
    ...
    ext::shared_ptr<FixedRateCoupon> cp =
        ext::dynamic_pointer_cast<FixedRateCoupon>(trade->coupons()[0]);
...
void example4() {
    ...
    riskFree.linkTo(
        ext::shared_ptr<YieldTermStructure>(new FlatForward(cachedToday, 0.02, Actual360())));

    Date expiryDate = calendar.advance(cachedToday, 9, Months);
    Date startDate = calendar.advance(expiryDate, 1, Months);
    ...
    Schedule schedule(startDate, maturity, Period(Quarterly), calendar, convention, convention,
    ...
    CreditDefaultSwap swap(Protection::Seller, notional, 0.001, schedule, convention, dayCounter);
    swap.setPricingEngine(swapEngine);
    Rate strike = swap.fairSpread();

    Handle<Quote> cdsVol(ext::shared_ptr<Quote>(new SimpleQuote(0.20)));

    ext::shared_ptr<CreditDefaultSwap> underlying(new CreditDefaultSwap(
        Protection::Seller, notional, strike, schedule, convention, dayCounter));
    underlying->setPricingEngine(swapEngine);

    ext::shared_ptr<Exercise> exercise(new EuropeanExercise(expiryDate));
    CdsOption option1(underlying, exercise);
    option1.setPricingEngine(ext::shared_ptr<PricingEngine>(
        new BlackCdsOptionEngine(defaultProbability, recoveryRate, riskFree, cdsVol)));

    Real cachedValue = 270.976348;
    if (std::fabs(option1.NPV() - cachedValue) > 1.0e-5)
        std::cout << "failed to reproduce cached value:\n"
                  << std::fixed << std::setprecision(6) << "    calculated: " << option1.NPV()
                  << "\n"
                  << "    expected:   " << cachedValue;

    underlying = ext::make_shared<CreditDefaultSwap>(Protection::Buyer, notional, strike, schedule,
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        if (example == 0 || example == 5) {
            std::cout << "***** Running ECIR CDS Option Case #1 *****" << std::endl;
            case1();
            std::cout << "***** Running ECIR CDS Option Case #2 *****" << std::endl;
            case2();
            std::cout << "***** Running ECIR CDS Option Case #3 *****" << std::endl;
            case3();
            std::cout << "***** Running ECIR CDS Option Case #4 *****" << std::endl;
            case4();
            std::cout << "***** Running ECIR CDS Option Case #5 *****" << std::endl;
            case5();
            std::cout << "***** Running ECIR CDS Option Case #6 *****" << std::endl;
            case6();
            std::cout << "***** Running ECIR CDS Option Case #7 *****" << std::endl;
            case7();
            std::cout << "***** Running ECIR CDS Option Case #8 *****" << std::endl;
            case8();
        }
